    ...
    <div className="border border-white/10 bg-[#12161F] py-8 sm:py-10 px-4 sm:px-6 text-white rounded-3xl my-8 shadow-xl">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-white/10 pb-4 mb-6 gap-3">
          <div>
            <span className="text-xs font-black uppercase tracking-widest text-[#BF953F] flex items-center gap-1.5">
              <MapPin className="h-4 w-4 text-[#BF953F]" /> Pan-India Statutory Directory
            </span>
          ...
          <Link
            href="/states"
            className="text-xs font-black text-[#BF953F] hover:text-white transition-colors whitespace-nowrap"
          >
          ...
          {REGION_GROUPS.map((group) => {
            ...
            return (
              <div key={group.name} className="space-y-2">
                <span className="text-xs font-black text-[#BF953F] block border-b border-white/10 pb-2 uppercase tracking-wider">
                  {group.name}
                </span>
                  ...
                  {groupStates.map((st) => (
                    <li key={st.slug}>
                      ...
                      >
                        ...
                        <span className="text-[11px] text-slate-400 group-hover:text-[#BF953F] font-mono">
                          {st.capital}
                        </span>
                      </Link>
                    </li>
